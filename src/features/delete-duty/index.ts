...
@injectable()
export default class DeleteDuty extends AbstractFeature<IParameters, IResponse> {
    constructor(
        @inject(Types.AccountDataSource) private readonly accountDataSource: Pick<IAccountDataSource, 'get'>,
        @inject(Types.DutyDataSource) private readonly dutyDataSource: Pick<IDutyDataSource, 'get' | 'delete'>,
    ) {
        super(schema)
    }
    ...
    async process(parameters: IParameters, options?: IOptions): IResponse {
        ...
        const deletedDuty = await this.dutyDataSource.delete({
            id,
        }, {
            transaction,
        })
