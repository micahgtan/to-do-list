...
describe('UpdateAccountEndpoint', (): void => {
    ...
    const mockValidEvent = {
        ...
        body: JSON.stringify(mockValidUpdateAccountParameters),
    }
    const mockInvalidUpdateAccountParameters = {
        first_name: 'updated_first_name',
    }
    const mockInvalidEvent = {
        ...
        body: JSON.stringify(mockInvalidUpdateAccountParameters),
    }
    ...
    describe('#handler', (): void => {
        ...
        it('updates an account record in the database if the parameters are valid', async (): Promise<void> => {
            await handler(mockValidEvent)
            const [account]: IAccount[] = await knex(ACCOUNTS_TABLE).where('id', mockAccount1.id)
            expect(account.first_name).toEqual(mockValidUpdateAccountParameters.first_name)
        })
